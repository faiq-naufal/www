...
Every redirection configuration in Cloudflare requires us to add the DNS through Cloudflare. I said earlier when the visitors visit the www version of my website, the visitors got unreachable URL. That was because the DNS for the www version not added and configured properly from the DNS records. If we want to redirect from www version to non-www version, we need to add the www subdomain to the DNS records.
